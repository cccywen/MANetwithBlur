...
class PAM_Module(nn.Module):
    def __init__(self, in_places, scale=8, eps=1e-6):
        ...
        # 修改输入通道（上一模块的输出通道）（改大）
        self.query_conv = nn.Conv2d(in_channels=in_places, out_channels=in_places, kernel_size=1)
        # self.query_conv = nn.Conv2d(in_channels=in_places, out_channels=in_places // scale, kernel_size=1)
        self.key_conv = nn.Conv2d(in_channels=in_places, out_channels=in_places, kernel_size=1)
