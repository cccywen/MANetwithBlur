...
import timm
from backbones.aspp import ASPP, ASPP_Bottleneck
...
class STL(nn.Module):
    def __init__(self, in_channel):
        super().__init__()
        # 将f1 f2都下采至32×32（用pooling）(之后尝试)
        # 先下采至128经两模块后再下采一次
        self.conv_start1 = ConvBNReLU(in_channel, 256, 1, 1, 0)
        # self.conv_start2 = ConvBNReLU(in_channel2, 256, 1, 1, 0)
        self.tem = TEM(128)
        self.ptfem = PTFEM()
    ...
    def forward(self, x1, x2):
        x1 = self.maxpool(x1)   # 试一下maxpooling和avgpooling
        # x2 = self.conv_start2(x2)
        x = torch.cat([x1, x2], dim=1)
        x = self.conv_start1(x)
        x_tem = self.tem(x)
...
class DeepLabV3(nn.Module):
    def __init__(self, num_classes):
        ...
        filters = filters = self.backbone.feature_info.channels()
        ...
        # self.r1 = resnet.layer1
        # self.r2 = resnet.layer2
        # self.r3 = resnet.layer3
        # self.r4 = resnet.layer4

        self.num_classes = 20

        # self.resnet = resnet101()  # specify the type of ResNet here
        self.aspp = ASPP(num_classes)  # if you use ResNet50-152, set self.aspp = ASPP_Bottleneck(num_classes=self.num_classes) instead
        ############改成MANet的上采样#############
        self.channel_redu = nn.Conv2d(774,6,1,1,0)

    def forward(self, x):
        # (x has shape (batch_size, 3, h, w))
        f1, f2, f3, res_out = self.backbone(x)

        stl_out = self.stl(f1, f2)

        h = x.size()[2]
        w = x.size()[3]

        aspp_out = self.aspp(res_out)  # (shape: (batch_size, num_classes, h/16, w/16))
        out = torch.cat([aspp_out, stl_out], dim=1)

        output = F.upsample(out, size=(h, w), mode="bilinear")  # (shape: (batch_size, num_classes, h, w))

        output = self.channel_redu(output)
        return output