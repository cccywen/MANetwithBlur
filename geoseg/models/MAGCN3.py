...
# 输入到PAM的feature map channel数小（类别数）
...
class BR1_Block(nn.Module):
    def __init__(self, input_channels, num_channels):
        super(BR1_Block, self).__init__()
        self.bn1 = nn.BatchNorm2d(input_channels)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv1 = nn.Conv2d(input_channels, num_channels, 3, padding=1)
...
class BR2_Block(nn.Module):
    def __init__(self, num_channels):
        super(BR2_Block, self).__init__()
...
class GCN(BaseModel):
    def __init__(self, num_classes, in_channels=3, pretrained=True, use_resnet_gcn=False, backbone='resnet50',
        ...
        self.gcn1 = GCN_Block(num_filters, resnet_channels[0], resnet_channels[0]//8)
        self.attention1 = PAM_Module(resnet_channels[0]//8)
        self.br1 = BR1_Block(resnet_channels[0]//8, num_classes)

        self.gcn2 = GCN_Block(num_filters, resnet_channels[1], resnet_channels[1]//8)
        self.attention2 = PAM_Module(resnet_channels[1] // 8)
        self.br2 = BR1_Block(resnet_channels[1]//8, num_classes)

        self.gcn3 = GCN_Block(num_filters, resnet_channels[2], resnet_channels[2]//8)
        self.attention3 = PAM_Module(resnet_channels[2] // 8)
        self.br3 = BR1_Block(resnet_channels[2]//8, num_classes)

        self.gcn4 = GCN_Block(num_filters, resnet_channels[3], resnet_channels[3]//8)
        self.attention4 = PAM_Module(resnet_channels[3]//8)
        self.br4 = BR1_Block(resnet_channels[3]//8, num_classes)

        self.br5 = BR2_Block(num_classes)
        self.br6 = BR2_Block(num_classes)
        self.br7 = BR2_Block(num_classes)
        self.br8 = BR2_Block(num_classes)
        self.br9 = BR2_Block(num_classes)
    ...
    def forward(self, x):
        ...
        x1 = self.br1(self.attention1(self.gcn1(x1)))
        x2 = self.br2(self.attention2(self.gcn2(x2)))
        x3 = self.br3(self.attention3(self.gcn3(x3)))
        x4 = self.br4(self.attention4(self.gcn4(x4)))
