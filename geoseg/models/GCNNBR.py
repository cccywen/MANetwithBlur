...
class GCN_Block(nn.Module):
    def __init__(self, kernel_size, in_channels):
        ...
        inter_channel = in_channels//8
        self.conv11 = nn.Conv2d(in_channels, inter_channel, kernel_size=(kernel_size, 1), padding=(kernel_size//2, 0))
        self.conv12 = nn.Conv2d(inter_channel, inter_channel, kernel_size=(1, kernel_size), padding=(0, kernel_size//2))

        self.conv21 = nn.Conv2d(in_channels, inter_channel, kernel_size=(1, kernel_size), padding=(0, kernel_size//2))
        self.conv22 = nn.Conv2d(inter_channel, inter_channel, kernel_size=(kernel_size, 1), padding=(kernel_size//2, 0))
...
class GCN(BaseModel):
    def __init__(self, num_classes, in_channels=3, pretrained=True, use_resnet_gcn=False, backbone='resnet50', use_deconv=False,
        ...
        self.gcn1 = GCN_Block(num_filters, resnet_channels[0])
        self.br1 = BR_Block(32)
        self.gcn2 = GCN_Block(num_filters, resnet_channels[1])
        self.br2 = BR_Block(64)
        self.gcn3 = GCN_Block(num_filters, resnet_channels[2])
        self.br3 = BR_Block(128)
        self.gcn4 = GCN_Block(num_filters, resnet_channels[3])
        self.br4 = BR_Block(256)
        ...
        self.con1 = nn.Conv2d(128, 64, kernel_size=1)

        self.br5 = BR_Block(128)
    ...
    def forward(self, x):
        ...
        if self.use_deconv:
            ...
            x3 = F.interpolate(self.br5(x3), size=x2.size()[2:], mode='bilinear', align_corners=True)+ x4
